...
public class SetUtils<T> {
	...
	private List<List<T>> processLargerSubsets(List<List<T>> result, List<T> set, List<T> subset, int subsetSize,
			int nextIndex) {
	...
	public static <T> Collection<List<T>> generateCombinations(List<T> list, int size) {
		return combinations(list, size).collect(Collectors.toCollection(ArrayList::new));
	}

	public static <T> Stream<List<T>> combinations(List<T> l, int size) {
	    if (size == 0) {
	        return Stream.of(Collections.emptyList()); 
	    } else {
	        return IntStream.range(0, l.size()).boxed().
	            <List<T>> flatMap(i -> combinations(l.subList(i+1, l.size()), size - 1).map(t -> pipe(l.get(i), t)));
	    }
	}
